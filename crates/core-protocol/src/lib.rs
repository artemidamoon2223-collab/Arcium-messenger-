...
/// In-memory sessions keyed by local handle. Holds no durable state and is
/// not used by `mobile-ffi`, whose sessions live only in the encrypted store
/// through [`messaging::Messenger`]; using both for one session would create a
/// second, diverging authority over it.
