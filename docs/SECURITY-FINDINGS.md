...
F-1, F-6, F-7, F-8, and F-10 are done. The next code-affecting items the review flagged
as pre-freeze / pre-deploy:
...
5. Then: **F-4** server-set anchoring (with MXE deploy), **F-11/F-12** CI + stub
   fail-closed, **F-14/F-15/F-16** at devnet integration. (**F-8** and **F-10**,
   previously listed here, are now FIXED — see the table above.)
